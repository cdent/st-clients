...
function madlib_render_event(evt) {
    ...
    return "<p>" + sentence + 
        " <span  id='"+evt.at+"' class='madlib-ago'>" + getAgoString(then) + "</span></p>";
}

// Maybe this should go in behavior.js
function updateTimestamps()
{
    jQuery(".madlib-ago").each(
            function(index) {
                var then = new Date();
                then.setISO8601(jQuery(this).attr('id'));
                jQuery(this).text(getAgoString(then));
            }
    );
}
