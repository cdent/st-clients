# $Id: Restructured.pm 5450 2007-09-12 20:18:53Z mnodine $
	    ...
	    push(@dom, $self->system_message
		 (1, $source, $lineno,
		  qq(No directive entry for "$dname" in module "Text::Restructured::Directive".\nTrying "$dname" as canonical directive name.)));
            ...
            # warn 'Attempting to use ' . $directive;

	    eval("use Text::Restructured::Directive::$directive");

            # warn '$DIRECTIVES{' . $directive . '} '
            #    . ( defined $DIRECTIVES{$directive} ? 'is' : 'is NOT' ) . ' defined.';
...
#
# WNO: This is horrific. It's slightly-less horrific now that I've made
# the initial parsing neater, but it's still awful.
#
sub ParseCSV {
    ...
    # WNO: This prepares the @split array for the logic that follows,
    # and the resulting structure of the @split array is (essentially)
    # the same as it was before. However, building it this way fixes
    # several bugs that the original big-ugly-split produced.
    #
    for my $pre_line (split("\n", $string)) {

        # XXX: This is a very simple-minded split(), and should be replaced
        # by Text::CSV_XS handling to avoid problems with delimiters
        # inside quotes.
        #
        # Note that the split() uses a negative limit (-1),
        # to ensure that trailing empty fields are preserved.
        #
        my @pre_row = 
            map { ($_, $delim) }
                split( qr{\s*$delim\s*}, $pre_line, -1);;

        # If @pre_row contains entries, then replace the last
        # element (which will be a delimiter, from the map above)
        # with a newline. Again, this is all so that the logic
        # after this loop will be able to continue processing
        # @split in the same mind-numbingly weird way as before.
        #
        $pre_row[-1] = "\n"
            if @pre_row;
...
sub handle_directive {
    ...
    # WNO: BUGFIX: This is necessary to allow for directives that contain
    # characters illegal in Perl module names (like the dash in "on-include").
    # There's more to do to really make this air-tight, but this works
    # for now.
    #
    $name =~ tr/[A-Z].-/[a-z]__/;

    # warn 'Adding ' . lc($name) . ' to DIRECTIVES hash' . "\n";
